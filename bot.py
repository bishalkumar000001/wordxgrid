...
async def cmd_hint(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        await context.bot.send_message(
            chat_id=user.id,
            text=(
                f"🔍 <b>Your Hint (only you can see this)</b>\n\n"
                f"One unfound word: <b>{hint_text}</b>\n\n"
                f"Hints remaining this game: <b>{left}</b>"
            ),
            parse_mode=constants.ParseMode.HTML,
        )
        await update.message.reply_text(
            f"💬 <a href='tg://user?id={user.id}'>{display_name(user)}</a>, "
            f"hint sent to your private chat! "
            f"({left} hint{'s' if left != 1 else ''} left this game)",
            parse_mode=constants.ParseMode.HTML,
        )
    except TelegramError:
        # User hasn't started bot in PM — fall back to group (only they see it via reply)
        await update.message.reply_text(
            f"🔍 <a href='tg://user?id={user.id}'>{display_name(user)}</a>, "
            f"your hint: <b>{hint_text}</b>\n"
            f"({left} hint{'s' if left != 1 else ''} remaining)\n\n"
            f"<i>💡 Start me in private to get hints only you can see!</i>",
            parse_mode=constants.ParseMode.HTML,
        )
