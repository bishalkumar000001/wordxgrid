...
def _lb_keyboard(period: str, scope: str, chat_type: str) -> InlineKeyboardMarkup:
    """Build leaderboard keyboard with ✅ on the currently active period/scope."""
    def _btn(label: str, p: str, s: str) -> InlineKeyboardButton:
        active = (p == period and s == scope)
        text = f"{label} ✅" if active else label
        return InlineKeyboardButton(text, callback_data=f"lb:{p}:{s}")

    rows = []

    # Scope row — only relevant inside groups
    ...
    # Period row 1
    rows.append([
        _btn("📅 Today", "day",   scope),
        _btn("📆 Week",  "week",  scope),
        _btn("🗓 Month", "month", scope),
    ])
    # Period row 2
    rows.append([
        _btn("📊 Year",     "year", scope),
        _btn("🏆 All Time", "all",  scope),
    ])
...
def build_caption(words: list, found: list, mode: str) -> str:
    ...
    return (
        ...
        f"⏰ <b>10 minutes</b> reset on each guess!\n"
...
async def update_grid_photo(
    ...
    The Support Group button is always kept on the grid photo."""
    ...
    try:
        await bot.edit_message_media(
            ...
            reply_markup=_grid_keyboard(),   # always keep support button
        )
    except TelegramError as e:
...
def _reschedule_timeout(context, game_id: str, group_id: int):
    """Cancel existing timeout job and start a fresh 10-min countdown."""
...
async def cmd_help(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        ...
        "/hint — Get a hint (private popup, only you see it)\n"
        "/leaderboard or /lb — Global leaderboard\n"
        "/lb day · /lb week · /lb month · /lb year\n"
        "/stats — Your personal stats\n"
        "/help — This message\n\n"
        "📌 The grid is pinned when a game starts.\n"
        "🎨 Found words light up with coloured oval highlights!\n"
        "⏰ Timer resets to 10 minutes on every correct guess.\n"
...
async def _do_start_game(
    ...
    """Core game-start logic, shared by cmd_new and cb_play_again."""
    ...
    await log_to_group(
        ...
        f"👥 Group: <b>{chat_title}</b> (<code>{chat_id}</code>)\n"
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # ── Reset the 10-min countdown on every correct guess ────────────────────
    _reschedule_timeout(context, game["game_id"], chat.id)

    # ── Re-render pinned grid ────────────────────────────────────────────────
...
async def cmd_hint(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Store hint under a short token (callback_data limit = 64 bytes)
    ...
    # Auto-delete the /hint command message after 30 s
    if update.message:
        context.job_queue.run_once(
            _delete_messages,
            when=30,
...
async def cb_hint_reveal(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Show the hint ONLY to the user who pressed the button (show_alert popup)."""
    query     = update.callback_query
    ...
    await query.answer(
        ...
        f"({left} hint{'s' if left != 1 else ''} left this game)",
        show_alert=True,
    )

    # Remove so it can only be revealed once
...
async def cmd_leaderboard(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Default scope: "chat" in groups, "global" in private
    scope = "chat" if chat_type in ("group", "supergroup") else "global"

    await _send_leaderboard(
        update.message, context, period, scope,
        chat_id=chat.id, chat_title=chat.title or "This Chat",
        chat_type=chat_type, edit=False,
    )
...
async def cb_leaderboard(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    parts = query.data.split(":")   # lb:{period}:{scope}
    if len(parts) != 3:
        return
    period = parts[1]
    scope  = parts[2]
