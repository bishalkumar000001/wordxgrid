...
def track_group(chat_id: int, title: str, username: str = "", member_count: int = 0):
    """Upsert a group the bot is active in."""
    _get_db().groups.update_one(
        {"chat_id": chat_id},
        {"$set": {
            "chat_id":      chat_id,
            "title":        title or "",
...
def get_all_groups() -> list:
    """Return all active groups the bot is in."""
    return list(_get_db().groups.find({"active": True}, {"_id": 0}))
