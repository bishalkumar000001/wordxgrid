...
# 12 solid highlight colors — vivid fills like in the reference screenshot
HIGHLIGHT_COLORS = [
    (220, 50,  50),    # red
    (255, 200, 0),     # yellow
    (50,  180, 80),    # green
...
def render_grid_image(
    ...
    padding    = 16
    title_height = 48
    border     = 2        # outer grid border thickness
    ...
    draw.rectangle([0, 0, img_w, title_height], fill=(30, 30, 30))
    title_text = f"🎮  {title}  🎮"
    try:
    ...
    draw.rectangle([gx0, gy0, gx1, gy1], outline=(60, 60, 60), width=border)

    # ── Build highlight map: (r, c) -> RGB color ─────────────────────────────
    highlight_map: Dict[Tuple[int, int], Tuple[int, int, int]] = {}
    ...
    for r in range(rows):
        for c in range(cols):
            ...
            if (r, c) in highlight_map:
                # Solid color fill for found word cells
                fill_color = highlight_map[(r, c)]
                letter_color = (255, 255, 255)   # white letter on colored bg
            else:
                fill_color = (255, 255, 255)      # white cell
                letter_color = (20, 20, 20)       # near-black letter
