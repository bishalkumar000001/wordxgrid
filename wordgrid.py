...
def render_grid_image(
    ...
    # ── Grid lines (subtle, drawn before cells) ───────────────────────────────
    for r in range(rows):
        for c in range(cols):
    ...
    # ── Highlight pills on a transparent overlay ──────────────────────────────
    if placed_words and found_words and word_order:
        ...
        for word in found_words:
            ...
            pill_color = (r_col, g_col, b_col, 210)   # semi-transparent

            wr, wc, dr, dc = placed_words[word]
            n = len(word)

            # Center of first and last letter cell
            ...
            pill_radius = cell_size * 0.42   # slightly inside cell edges
    ...
    # ── Letters — always black, drawn on top of pills ─────────────────────────
    for r in range(rows):
        for c in range(cols):
            x = padding + c * cell_size
            y = title_height + padding + r * cell_size
            ...
            draw.text((tx, ty), ch, fill=(10, 10, 10), font=font)
...
def get_hint_text(words: List[str], found: List[str]) -> str:
    """Word list shown in the grid caption — full word always visible."""
    lines = []
